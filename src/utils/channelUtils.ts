...
export function getLastPopChannel(): Channel | null {
    return null;
}
...
export async function scheduleWindowMessages(
    channelId: string,
...
export async function getMessageData(
    channel: TextChannel
): Promise<MessageWithDisplayName[]> {
    const allMessages: Message[] = [];
    const allMessagesData: MessageWithDisplayName[] = [];
    let lastMessageId: string | undefined;

    while (true) {
        ensureRestToken(channel.client);
...
export async function sendNextCamp(message: Message): Promise<void> {
    ...
    if (client.lastPopChannel) {
        ...
        if (client.lastPopChannel instanceof TextChannel) {
            ensureRestToken(client);
            await client.lastPopChannel.send(
                `Here's the link to the next POP: ${messageLink}`
            );
        }
    }
